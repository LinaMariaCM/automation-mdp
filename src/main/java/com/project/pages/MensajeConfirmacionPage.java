...
import com.mutuaPropietarios.testCasesData.Helpers.DownloadLinkHelper;
...
public class MensajeConfirmacionPage
{
	...
	@FindBy(css = "#form1 > table.grid.narrowBox > tbody > tr:nth-child(3) > td:nth-child(3) > a > img")
			...
			String Path) throws IOException
	{
		...
		File tempFolder = new File(this.browserContext.getProperties().fileDownloadTempPath);
		...
		String folderName = "\\polizaNumero_" + this.tData.getNumPoliza() + "_Documentos_Suplemento" + dateTime;
		...
		downloads.stream().forEach(p ->
		{
			...
			if (this.CheckIfFilesInFolderHaveBeenDownloaded(tempFolder))
			{
				try
				{
					String[] extensions =
					{ "pdf" };
					Collection<File> files = FileUtils.listFiles(tempFolder, extensions, true);
				...
				{
					logger.trace("Ha habido un error al mover el archivo descargado a la carpeta de destino", e);
				}
			}
		});
	...
	public List<DownloadLinkHelper> getDownloadLinksAndNames()
	{
		...
		this.rowWithDocument.stream()
				.forEach(p -> downloads.add(new DownloadLinkHelper(p.findElement(By.xpath(this.xpathRowLinkDescriptionFilter)).getText(),
	...
	public void CheckIfPageHasLoadedCorrectly()
	{
		...
		this.wh.switchToFrame(this.cuerpoFrame);
		Assert.assertTrue("La pagina con el listado de documentos no ha cargado correctamente", this.rowWithDocument.size() > 2);
		this.wh.exitFromFrame();
		logger.debug("END - CheckIfPageHasLoadedCorrectly");
	}
	
	public void searchTextInCopiaTomadorPDF() // Copy contents of Copia Tomador PDF to Notepad and search for the text of the motivos suplemento.
	{
		this.wh.switchToFrame(this.cuerpoFrame);
