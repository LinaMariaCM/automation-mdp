...
public class MediadoresDGSPage extends PageObject {
	...
	public MediadoresDGSPage envioMediadorDGS() {
		...
		clickBuscar();
		webDriver.waitWithDriver(3000);
		seleccionarMediadorEnvioDGS(getTestVar(Constants.ID_MEDIADOR_ALTA).toString()); // ir a resultados mediante método para localizarlos y seleccionar el mediador que toca. Revisar si hay otra lógica para los auxiliares, prueba con AE
		clickEnviarDGS();
		obtenerNumDGS();
		debugEnd();
		return this;
	...
	public MediadoresDGSPage seleccionarMediadorEnvioDGS(String idMediadorAltaDGS) {
		...
		for(int i = 1; i < obtenerListaEnvioDGS.size(); i++) {
			...
			//	Assert.assertTrue(checkMediadorValue, "Comparar campos: la Id del mediador NO coincide");
				webDriver.clickElementChildByAttributeInFrame(By
					.cssSelector("#formDatosEnvio > table.grid.widthstd > tbody > #tr" + (i + 1) + " > td:nth-child(2)"), cuerpoFrame, "value", idMediadorAltaDGS); // > input quitado porque es child element
			}
		}
