...
public class AltaAperturaDeclaracionSiniestrosPage extends PageObject {
	...
	public AltaAperturaDeclaracionSiniestrosPage datosPersonaExtra(String rol, String nombre, String apellido1, String apellido2, String tipoDocumento,
		...
		webDriver.clickElementFromDropDownByIndex(rolDrpDwn, 8);
		...
		// TODO Comprobar algoritmo, si hay noEmail se deberia de ahcer click en el checkBox de noEmail?
		if(noEmail != null && !noEmail.isEmpty()) {
			email = "prueba@esto.es";
		}
		...
		if(riesgoAsegurado != null && !riesgoAsegurado.isEmpty()) {
			...
			if(tipoVia != null && !tipoVia.isEmpty()) {
				webDriver.clickElementFromDropDownByAttribute(tipoViaDrpDwn, "value", tipoVia);
			}

			if(via != null && !via.isEmpty()) {
				webDriver.setText(viaInput, via);
			}

			if(numero != null && !numero.isEmpty()) {
				webDriver.setText(numeroInput, numero);
			}

			if(piso != null && !piso.isEmpty()) {
				webDriver.setText(pisoInput, piso);
			}

			if(puerta != null && !puerta.isEmpty()) {
				webDriver.setText(puertaInput, puerta);
			}

			if(cp != null && !cp.isEmpty()) {
				webDriver.setText(codPostalInput, cp);
			}
			...
			if(provincia != null && !provincia.isEmpty()) {
				webDriver.clickElementFromDropDownByAttribute(provinciaDrpDwn, "value", provincia);
			}
		}
